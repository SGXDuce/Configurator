...
**(Batch 47, regenerated batch 48)** The example above is not hand-written — it's the traced output of the real export logic (`buildAssemblyPresetTree`, `buildSlidingWinOverlapTree`, `collectExportGeometry`, `computeCrossElevationLinks`, `buildExportElevation`, and, as of batch 48, `computeSlidingAssemblyLeafCorrections`, all as committed) against a concrete, numerically-simulated session, the same tracing method used to verify each batch throughout this project. Earlier versions of this example had drifted through hand edits across batches 39–46 into combinations the real code can't actually produce (a sashless slider against a meeting jamb, which `leafAllowedAgainstMeetingEdge` blocks; a `linkedPaneId` pointing within the same elevation; a slider edge flagged unframed by no real case; a fixed pane missing its Case 4 flag next to a sashless slider; pane widths exceeding the opening) — batch 47 fixed all of those. **Batch 48 regenerated the same session's numbers again**, this time also correcting for the real-panel-size bug found in that same pass: the assembly's own `O` leaf (`".L.R"`) was exporting its DRAWN width (452mm), not its real, entered panel width (458mm) — see "Real panel size for sliding-window/door assemblies" above for the full derivation.

The traced session: **Elevation 1** is a sashless `OX-win` (910mm nominal row width, `hasFrame:true`; `computeSlidingWinPrefillWidths` with `SASHLESS_FRAME_TUCKIN_MM=6` solves real panel widths O=458mm/X=518mm — the actual entered/made panel sizes, not the drawn footprint), sitting beside a separate, ordinary `type:'fixed'` pane (500mm wide) placed against the elevation's meeting jamb (`jambR`, real+window, satisfying `leafAllowedAgainstMeetingEdge`). Pane `".L.R"` is the assembly's own `O` — `widthMM: 458` (the real panel width, batch 48 — drawn width was 452mm before this batch's correction) and `xMM: 57` (`3mm` less than the drawn `xMM` would be, since `SASHLESS_FRAME_TUCKIN_MM/2 = 3mm` of this panel's own left edge genuinely sits inside `jambL`'s cavity); still all-zero sash (batch 45) and `unframedEdges.right: true` via Case 4 — its matched overlap-join neighbour `".L.S"` is sashless — computed from the ORIGINAL drawn geometry, unaffected by the width/`xMM` correction. Pane `".L.S"` is the assembly's `X` — `widthMM: 518` (its drawn width already equalled its real width, per batch 48's own derivation — every `X` leaf's drawn width is always correct, only `O` ever needs correcting), `sashless: true`, `SASHLESS_STILE_MM=10` left/right, all-zero top/bottom, no Case 4 (that only ever applies to `type:'fixed'` leaves). Pane `".R"` is the separate meeting-jamb `O` (not part of any sliding assembly, so entirely untouched by batch 48) — `unframedEdgeCount: 1` and `unframedEdges.right: true` via Case 1 (touches the meeting-tagged `jambR`), linked to Elevation 2's pane `"F"`. **Elevation 2** is a single `800×900mm` fixed pane spanning its whole opening, touching its own meeting jamb (`jambL`, the opposite side, per `addAngledJoin`'s always-opposite-jambs convention) — `unframedEdgeCount: 1`, `unframedEdges.left: true` via Case 1, linked back to Elevation 1's `".R"`. Both elevations share `900mm` opening height, satisfying `computeCrossElevationLinks()`'s edge-position match (`edgeStart`/`edgeEnd` both `0`/`900`, well within `PANE_EDGE_MATCH_TOL_MM` — computed from the elevations' own original drawn geometry, a separate `collectExportGeometry` pass entirely unaffected by batch 48). `frameMembersMM.jambR`/`jambL` are `0` on the respective meeting-tagged side (`getMembers()` always zeroes a meeting-tagged member, regardless of the elevation's own stored `frameMembers` value) — everywhere else the real `60mm` frame members apply. `frameLengthMM` is the real Part-5 sum: Elevation 1 = `2836` (`.L.S`'s own sash perimeter, `2×(518+900)` — uses the real `widthMM`, though for this pane that equals the drawn width anyway; `.L.R`/`".R"` are both `fixed` and contribute `0` regardless of width) + `900` (the one real 60mm mullion between the assembly and `".R"`, a v-axis member whose length is the region height it divides) + `4080` (outer frame: head+sill each contribute `overallWidthMM` (`1530`), jambL contributes `overallHeightMM` (`1020`), jambR contributes `0` since it's meeting-tagged — `1530+1530+1020+0=4080`) = `7816`. Elevation 2 = outer frame only (single fixed pane, no sash perimeter, no internal mullion): head+sill each contribute `overallWidthMM` (`860`), jambR contributes `overallHeightMM` (`1020`), jambL contributes `0` (meeting-tagged) — `860+860+0+1020=2740`. `totalFrameLengthMM` is their sum, `10556`.
