...
### Version: 2.3 (supersedes v2.2 — covers batches 1 through 19, plus AS 1288 integration planning and the depth/glass data model)
...
- **Double-hung** — D, DD, DDD. Each unit is its own two-level subtree — **batch 19:** now a real `kind:'sashSplit', axis:'h', fromEdge:'start', sashSizeMM:topHeightMM, overlapMM:60` internal division (not a plain transom), joining a top `vertical-slider` (`slideDirection:'down'`, the sash — `children[0]`) with a bottom `vertical-slider` (`slideDirection:'up'`, the remainder — `children[1]`). `fromEdge:'start'` was verified against `walk()`'s actual sashSplit `axis:'h'` geometry (not assumed by analogy) before trusting it: it places `children[0]` at the smaller y (top of the region), matching the pre-existing plain-split convention where `children[0]` was already the top leaf. Multiple units still fold into a row the same way every other family's sections fold (unaffected — `foldSectionsIntoSplits` doesn't care what shape each "leaf" is). Unit width and top-pane height are each one shared value across all units; **the "Transom thickness" input is gone entirely** (batch 19) — the overlap is a fixed 60mm (`DOUBLE_HUNG_OVERLAP_MM`), same precedent as a sliding-overlap id having no mullion input. The between-unit mullion-thickness input still only appears when unit count > 1 (a real mullion between separate units, unaffected by this batch).
...
**Double-hung internal division — implemented in batch 19 (see §2.9, §5):** the top sash's own bottom rail (60mm) and the bottom sash's own top rail (60mm) genuinely interlock — built as `kind:'sashSplit', fromEdge:'start', overlapMM:60`, replacing the former plain `kind:'split'` transom.
