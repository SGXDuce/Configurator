...
- **Sliding windows** — OX, OXX, OXXO, XOX, OXXX (five — no OXXXX/OXXXXX window variant, and no window OXO, confirmed against Duce's actual price list, §7). `productClass:'window'`. **Batch 17:** the 4 core `-win` ids (`OX-win`, `OXX-win`, `XOX-win`, `OXXO-win`) now build with a genuinely different internal structure from their sliding-door namesakes — see the dedicated "Sliding-window overlap" write-up below. `OXXX-win` is unaffected by batch 17 (no real dimensional data exists for a 5-panel window run; it still uses the plain-`split` mullion mechanism, same as every sliding door id).
...
- **Double-hung** — D, DD, DDD. Each unit is its own two-level subtree (a `kind:'split', axis:'h'` internal transom splitting a top `vertical-slider` (`slideDirection:'down'`) from a bottom `vertical-slider` (`slideDirection:'up'`)). Multiple units fold into a row the same way every other family's sections fold. Unit width, top-pane height, and transom thickness are each one shared value across all units. The between-unit mullion-thickness input only appears when unit count > 1.
  ...
  - **Sliding-window overlap — implemented in batch 17, window ids only.** `buildAssemblyPresetTree` now takes an explicit `id` parameter (not derivable from `pattern`, since `'OX'` the sliding door and `'OX-win'` the sliding window share a pattern string) and branches to `buildSlidingWinOverlapTree` whenever `isSlidingWinId(id)` is true. Every fixed panel (`O`) is 600mm, every sliding panel (`X`) is 660mm (60mm wider — the sliding leaf always reaches over its neighbour, frontmost, never the fixed one). Each adjacent join is classified via `classifySlidingWinJoin` (reusing `computeDefaultSlideDirections`, the existing source of truth for which way each `X` points): an `O`-`X` join is always a 60mm overlap with `X` as the sash; an `X`-`X` join with both `X`s pointing the same direction is a 60mm overlap with the **farther-from-the-reference-O** `X` as the sash (frontmost); an `X`-`X` join with diverging directions is **flush** — no overlap at all, built as an ordinary zero-thickness `kind:'split'` (the pre-existing zero-thickness plain-split mechanism, not a new construct) rather than a sash of zero size, since the two panels genuinely meet stile-to-stile with no interlock. A converging `X`-`X` combination has no confirmed physical meaning and never occurs in any of the four confirmed patterns (`OX`/`OXX`/`XOX`/`OXXO`) — `classifySlidingWinJoin` throws rather than guessing if one is ever produced. The tree is built by splitting the pattern into maximal "overlap runs" separated by flush joins, building each run as its own left-to-right nested `sashSplit` chain (verified this never needs a multi-leaf accumulated node to serve as a sash for any of the four confirmed patterns — defensively throws if a future pattern ever required it), then joining separate runs with an ordinary (zero-thickness where flush) split. Catalog-informed prefill (§2.9/batch 15) is also `-win`-aware: `computeSlidingWinPrefillWidths` solves `O = (T − 60·n_X + 60·joins) / (n_O + n_X)`, `X = O + 60` against the real catalog total (`joins` counts only real overlap joins, excluding flush ones) — verified exact, no rounding, against every real catalog data point for `OX-win`/`OXX-win`/`OXXO-win`; `XOX-win` has no catalog entry at all, so it prefills with the plain 600/660 defaults directly. A `-win` id has no mullion input at all in its widths form (its joins are overlap/flush, never a separate mullion member) — its entered-total check instead uses `computeSlidingWinTotalWidth` (sum of section widths minus 60mm per real overlap join). "Edit assembly widths" reopen is also supported for `-win` presets: `assemblyPresetRef` now additionally stores `id` (not just `{pattern, productClass}`, since pattern alone can't distinguish a `-win` id from its plain sliding-door sibling), and a dedicated `computeSlidingWinSectionPaths` re-derives each section's real tree path (which is **not** the uniform right-fold `[1,1,...,0]` shape every other family uses, since a `-win` tree mixes `sashSplit` and flush-split nodes) for both `getAssemblyPresetCurrentState` and `getAssemblyLeafNodesInOrder`. Sliding **door** ids (`OX`, `OXX`, `OXXO`, `XOX`, no `-win` suffix) are completely untouched — they still build via the original plain-`split` mullion fold, since no real overlap data exists for doors.

**Reopening an assembly to edit its widths:** every family except double-hung stamps `assemblyPresetRef: {pattern, productClass, id}` (`id` added batch 17) on its top-level node at creation, and an "Edit assembly widths" toolbar button reopens the original form pre-filled with current live widths and the marked split's own current mullion thickness (or, for a `-win` id, no mullion — see above). Confirming a width edit preserves every section's existing leaf data exactly — only the split chain's positions/thickness (or, for `-win`, the whole overlap tree, rebuilt via `buildSlidingWinOverlapTree`) change — validated via `validateTreeAgainstSize`. **Batch 17 note:** a `-win` preset's top-level node can itself be a `sashSplit` rather than a plain `split` (e.g. `OX-win`/`OXX-win`/`XOX-win`, whose sections form a single overlap run with no flush join at all — only `OXXO-win`'s middle flush join produces a plain `split` at the top) — every place that previously assumed `node.kind === 'split'` to find/stamp/select this marker (`findAssemblyPresetAncestor`, the "selected bar" edit-button check, the marker-stamping in `confirmAssemblyPreset`) now also accepts `sashSplit`. Double-hung deliberately does not support reopen yet (its two-level shape doesn't fit the flat-row reopen assumption); the button is genuinely absent, not disabled, for a double-hung selection.
...
- ~~Sliding assembly overlap (60mm, X-axis) has real data but has not been adopted into `buildAssemblyPresetTree`~~ — **implemented in batch 17** for the 4 core `-win` ids (see §2.9). Sliding **door** ids remain plain-`split`, unchanged (no real overlap data exists for doors).
